...
- 校验 = 内核 `model.validate`(裸端口 / 交叉连线 / 引用存在性 / 类型兼容 + 静态提示);
- 编辑操作 = 内核 `engine.edit.apply_edits`(增删节点、增删连线、改配置、换实现);
- 预览 = 内核 `engine.World` **真实运行**(headless tick,同步轮次确定性可复现 + RNG seed)——**编辑器天然是调试器**,预览不需要 dry-run 模式;
...
打开 `http://localhost:5173`:从内置节点白名单(Clock/Counter/Threshold/…)拖入画布 → 连线(数据/控制端口着色区分,交叉连线被拒绝)→ 配置节点 → 预览运行。
...
| `POST /api/preview` | headless 预览:确定性运行 N 拍 → 逐拍快照 + 状态/端口 held + 日志 |
...
{"op": "add_edge",    "wire": {"src_node": "clock", "src_port": "count", "dst_node": "counter", "dst_port": "increment"}}
{"op": "remove_edge", "wire": {…}}
...
V0:内置白名单节点 + 单图画布 + 校验报告 + 一次性预览(每次运行从第 0 轮开始)。

后续(独立演化):
1. 预览会话(单步/暂停/恢复,编辑器作为调试器的完整形态);
