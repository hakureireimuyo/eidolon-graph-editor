...
V0:内核节点库(一节点一文件,含 Output 日志输出 / Input 手动输入 / Random 随机函数)+ 单图画布 + 事件驱动运行(顶栏右侧平铺「运行/暂停/结束」,世界自驱——源节点按自身规则发事件,Clock 默认每秒一次;暂停 = 传播闸门;运行中图锁定编辑;「文件」菜单:新建/载入/重命名/保存/删除;种子每图自动随机)。底部控制台三 tab:控制台输出 / 问题(编译检查只在点「运行」时做一次,校验不通过不运行、错误进此 tab)/ 节点(选中节点的状态与缓存,视图按节点类型可重载);左右面板与控制台均可收起展开。编辑器不定义节点——节点全部归属内核,编辑器只对特殊节点做展示对接(读 Output 状态喂控制台、为 Input 渲染输入栏)。
